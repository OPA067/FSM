...
from .module_PerturbedTopK import PerturbedTopK
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        embed_dim = state_dict["text_projection"].shape[1]
        self.max_word, self.max_frame = self.config.max_words, self.config.max_frames
        self.h_max_frame = self.max_frame // 2
        self.h_max_patch = self.max_frame * 4
        self.alpha, self.beta = self.config.alpha, self.config.beta

        self.v_ptk_f = PerturbedTopK(k=self.h_max_frame)
        self.v_ptk_p = PerturbedTopK(k=self.h_max_patch)
    ...
    def forward(self, text, text_mask, video, video_mask, idx=None, global_step=0):
        ...
        ###### Step-I: Select f_feat ######
        sims_sf = torch.einsum("bd,bfd->bf", [self.norm(s_feat), self.norm(f_feat)])
        sims_sf = torch.softmax(sims_sf, dim=-1)
        sims_sf = self.v_ptk_f(sims_sf) # [b, k, f]
        ...
        ###### Step-II: Select p_feat ######
        p_feat_h = torch.einsum("bkf,bfpd->bkpd", [sims_sf, p_feat.reshape(b, f, -1, d)]).reshape(b, -1, d)
        sims_wp = torch.einsum("bwd,bpd->bwp", [self.norm(w_feat), self.norm(p_feat_h)])
        sims_wp, _ = sims_wp.max(dim=-2) # [b, w, p] -> [b, p]
        sims_wp = torch.softmax(sims_wp, dim=-1)
        sims_wp = self.v_ptk_p(sims_wp)  # [b, k, p]
        p_feat_h = torch.einsum("bkp,bpd->bkd", [sims_wp, p_feat_h])
        ...
        total_loss = loss_sf_h + loss_wp_h  \
                   + loss_sf + loss_wp      \
                   + loss_kl_sf + loss_kl_wp
