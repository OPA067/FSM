...
class PerturbedTopKFuntion(torch.autograd.Function):
    ...
    def forward(ctx, x, k: int, num_samples: int = 500, sigma: float = 0.05):
        b, p = x.shape
        noise = torch.normal(mean=0.0, std=1.0, size=(b, num_samples, p)).to(dtype=x.dtype, device=x.device)
        perturbed_x = x.unsqueeze(1) + noise * sigma
        topk_results = torch.topk(perturbed_x, k=k, dim=-1, sorted=False)
        indices = topk_results.indices
        indices = torch.sort(indices, dim=-1).values

        perturbed_output = F.one_hot(indices, num_classes=p).float()
...
p_feat = torch.rand(32, 12*50, 512)
